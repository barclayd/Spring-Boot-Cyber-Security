(function () {
	...
	var url = 'http://rob:abcd1234@www.domain.com/path/charity.html?query1=test&silly=willy#test=hash&chucky=cheese';
	var urlHttps = 'https://rob:abcd1234@www.domain.com/path/charity.html?query1=test&silly=willy#test=hash&chucky=cheese';
	var urlIp = 'https://rob:abcd1234@1.2.3.4/path/charity.html?query1=test&silly=willy#test=hash&chucky=cheese';
	...
	test('path', function () {
		deepEqual(window.url('path', url), '/path/charity.html');
	...
	test('query string', function () {
		...
		deepEqual(window.url('?', 'http://domain.com?silly=willy&field[0]=zero&field[1]=one'), {
			'silly': 'willy',
			'field': ['zero', 'one']
		});
		deepEqual(window.url('?', 'http://domain.com?silly=willy&field[0]=zero&field[2]=two'), {
			'silly': 'willy',
			'field': ['zero', undefined, 'two']
		});
		...
		deepEqual(window.url('?', 'http://domain.com?field[0]=zero&field[1]=one&var=test'), {
			'field': ['zero', 'one'],
			'var': 'test'
		});
	});
	...
	if (typeof jQuery !== 'undefined') {
		test('jQuery', function () {
			...
			deepEqual($.url('path', url), '/path/charity.html');
